...
```bash
aws lambda create-function --function-name micronaut-aws-issue-274 \
...
```bash
aws lambda invoke --function-name micronaut-aws-issue-274 --payload '{"resource": "/{proxy+}", "path": "/ping", "httpMethod": "GET"}' build/response.txt
